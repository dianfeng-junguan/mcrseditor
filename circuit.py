    ...
    def mouseReleaseEvent(self, event):
        if select.mode == SelectMode.GATE and self.gate_item:
            ...
            for gate in current_circuit.gates:
                if gate[0].x<=grid_pos.x<=gate[0].x+gate[1].size[0]*render_config.grid_spacing and gate[0].y<=grid_pos.y<=gate[0].y+gate[1].size[1]*render_config.grid_spacing:
                    current_circuit.remove_gate(gate)
                    print("remove gate")
                    break
            for wire in current_circuit.wires:
                if wire[0].x<=grid_pos.x<=wire[1].x and wire[0].y<=grid_pos.y<=wire[1].y:
